...
const ProjectCard = memo(function ProjectCard({
  ...
  return (
    <div
      className=" group relative h-[520px] max-sm:w-[310px] max-w-[350px] mx-auto flex flex-col rounded-2xl 
    ...
    >
      {/* badge */}
      <span
        className={`${config.class} border z-10 absolute top-3.5 left-3.5 text-xs font-semibold px-2 py-1 rounded-md backdrop-blur-md`}
      >
        {config.label}
      </span>
      ...
      <div className="h-full overflow-hidden p-2 font-fira-code ">
        ...
        <div className="flex flex-wrap gap-1.5 my-4">
          {labels.slice(0, 4).map((item, i) => (
            <span
              className="bg-orange-700/20 border border-orange-700/30 border border-orange-700/30 px-2 py-0.5 rounded-lg text-xs text-orange-200/80"
              key={item.value + i}
            >
              {item.label}
            </span>

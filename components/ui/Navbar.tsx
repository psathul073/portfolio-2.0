...
import { getNextRoute, getPrevRoute } from "@/helper/navigation";

const NAV_ITEM_CLASS =
  "relative inline-flex items-center justify-between py-2.5 px-4 backdrop-blur-md rounded-full border border-white/10 shadow-inner hover:-translate-y-[2px] hover:shadow-[0_10px_30px_rgba(255,159,28,0.35)] transition-all duration-200 cursor-pointer select-none";
const DEMO_NAV_ITEM_CLASS =
  " shrink-0 text-orange-50/90 mt-3 bg-amber-50/20 p-1 rounded-full backdrop-blur-xs cursor-pointer ";
const ICON_SIZE = 30;
...
const NavItem = memo(
  ({
    ...
    <motion.div variants={itemVariants}>
      <div
        className={`${NAV_ITEM_CLASS} ${width} mt-4 ${className} hover:scale-105 active:scale-95 transition-transform`}
      >
        <Link
          href={href}
          className="w-full hover:opacity-80 transition-opacity text-orange-50/90"
        >
          {label}
        </Link>
    ...
    return (
      <div className="fixed inset-0 h-fit inline-flex justify-center gap-5 z-50">
        <button
          disabled={!prevRoute}
          title="Previous page"
          aria-label="Prev Route Icon"
          onClick={() => prevRoute && router.push(prevRoute)}
          className={`${DEMO_NAV_ITEM_CLASS} disabled:cursor-not-allowed disabled:text-orange-50/50 disabled:bg-amber-50/15`}
        >
          <ChevronsLeft size={24} />
        </button>
        <button title="Open navbar" aria-label="Menu Icon" onClick={activeNav}>
          <ChevronsDownIcon size={ICON_SIZE} className={DEMO_NAV_ITEM_CLASS} />
        </button>
        <button
          disabled={!nextRoute}
          title="Next page"
          aria-label="Next route Icon"
